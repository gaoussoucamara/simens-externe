...
class PatientTable {
	...
	public  function updatePatient($donnees, $id_patient, $date_enregistrement, $id_employe, $numero_dossier){
	...
	public function getListePatient(){
		...
		$aColumns = array('NUMERO_DOSSIER', 'Nom','Prenom','Datenaissance', 'Adresse', 'id', 'id2');
	...
	/*
	public function listeServiceChirurgie(){
		$adapter = $this->tableGateway->getAdapter();
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('ser_ch' => 'service_chirurgie'));
		$select->columns(array('ID_SERVICE','NOM'));
		$stmt = $sql->prepareStatementForSqlObject($select);
		$result = $stmt->execute();
