...
class AdmissionTable {
	 ...
	 */
	public function verifierPatientConsulter($idPatient){
		$today = new \DateTime ( 'now' );
		$date = $today->format ( 'Y-m-d' );
	...
	public function deleteAdmissionPatient($id, $idPatient){
		if($this->verifierPatientConsulter($idPatient)){
		    return 1;
		} else {
