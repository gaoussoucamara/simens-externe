...
/*****************************************************************************************************************************/
	...
	public function admissionAction() {
		...
		//var_dump("sss");exit();
		$formAdmission->get('which_medcin')->setValueOptions($this->getConsultationTable()->ListeMedecin());
		...
		if ($this->getRequest ()->isPost ()) {
			...
			$html .= "<div id='' style='color: green; opacity: 0.09; float:left; margin-right:10px; margin-left:5px; margin-top:5px;'>".$unPatient['NUMERO_DOSSIER']."</div>";
	...
	public function enregistrerAdmissionAction() {
		...
		$date_admission = $today->format ( 'Y-m-d' );
		$date_enregistrement = $today->format ( 'Y-m-d H:i:s' );
		
		$id_patient = ( int ) $this->params ()->fromPost ( 'id_patient', 0 );
		$medecin = $this->params()->fromPost('which_medcin');
		//var_dump($medecin);exit();
	...
	public function listePatientsAdmisAction() {
		...
		*/
		...
		//var_dump($listePatientsCons); exit();
		...
		return new ViewModel ( array (
				'listePatientsAdmis' => $listePatientsAdmis,
				'form' => $formAdmission,
				'listePatientsCons' => $listePatientsCons,
		) );
	}
...
// 		var_dump($tabURI[0].'public/'); exit();
	...
	public function enregistrementAction() {
		...
		if (isset ( $_POST ['terminer'] ))  // si formulaire soumis
		{
			...
			if ($img != false) {
				...
				imagejpeg ( $img, $tabURI[0].'public/img/photos_patients/' . $nomfile . '.jpg' );
	...
	public function enregistrementModificationAction() {
		...
		if (isset ( $_POST ['terminer'] )) 	
		{
			...
			if ($img != false) {
				...
				if($ancienneImage) {
					unlink ( $tabURI[0].'public/img/photos_patients/' . $ancienneImage . '.jpg' );
				}
				imagejpeg ( $img, $tabURI[0].'public/img/photos_patients/' . $nomfile . '.jpg' );
				
				$donnees['PHOTO'] = $nomfile;
				$Patient->updatePatient ( $donnees , $id_patient, $date_modification, $id_employe, $numero_dossier);
	...
	public function supprimerAdmissionAction(){
		if ($this->getRequest()->isPost()){
			...
			$resultat = $this->getAdmissionTable()->deleteAdmissionPatient($id, $idPatient);
	...
	public function vuePatientAdmisAction(){
		...
		*/
		...
		$html .="<div id='titre_info_admis'>Informations sur l'admission <!--img id='button_pdf' style='width:15px; height:15px; float: right; margin-right: 35px; cursor: pointer;' src='".$this->baseUrl()."public/images_icons/button_pdf.png' title='Imprimer la facture' --></div>";
		$html .="<div id='barre_separateur'></div>";
