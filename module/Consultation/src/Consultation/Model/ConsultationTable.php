...
class ConsultationTable {
	...
	//Liste des medecins que l'agent verra pour l'orientation du patient vers le medecin determiné 
	//
	public function ListeMedecin(){
		...
		$select->join(array('em' =>'employe'), 'em.id_personne = p.id_personne',array());
		$select->join(array('u' =>'utilisateurs'), 'u.id_personne = em.id_personne',array());
		$select->join(array('m' =>'medecin'), 'm.id_employe = em.id_personne',array());
		
		$where = new Where();
		$medecin = "medecin";
		$where->equalTo('u.role', $medecin);
		...
		foreach ($result as $data) {
			...
			$options[$data['PRENOM']] = $nom_complet;
		}
	...
	public function getConsultationDuJour(){
		...
		$select = $sql->select ();
		$select->from( array( 'c' => 'consultation' ));
		$select->where(array('DATE_CONS' => $today));
	...
	public function addConsultation($values, $id_medecin){
		...
		try {
			...
			$dataconsultation = array(
					...
					'ID_ADMISSION'=> $values->get ( "id_admission" )->getValue (),
					...
					'GLYCEMIE_CAPILLAIRE' => $values->get ( "glycemie_capillaire" )->getValue (), 
					'DATE_CONS' => $date,
					'HEURE_CONS' => $values->get ( "heure_cons" )->getValue (),
			);
	 ...
	 * 
		...
		$select->where(array( 'DATEONLY' => $date, 'a.date_cons' => $date, 'c.ARCHIVAGE' => 0));
	 ...
	 */
	//liste des patients Ã  consulter par le medecin 
	public function listePatientsAdmis(){
		$today = new \DateTime();
		$date = $today->format('Y-m-d');
		
		/*
		 * Les patients admis et non encore consultés
		 */
		$adapter = $this->tableGateway->getAdapter ();
		$sql = new Sql ( $adapter );
		$select1 = $sql->select ();
		$select1->from ( array ('c' => 'consultation') );
		$select1->columns(array ('ID_PATIENT') )->where(array('DATE_CONS' => $date));	
		...
		$select->where(array('a.date_admission' => $date, new NotIn ( 'pers.ID_PERSONNE', $select1 )));
		$select->order('id_admission ASC');
		...
		$select->join(array('a' => 'admission'), 'pers.ID_PERSONNE = a.id_patient', array('Id_admission' => 'id_admission'));
		$select->join(array('c' => 'consultation'), 'c.ID_ADMISSION = a.id_admission', array('*'));
		$select->where(array('a.date_admission' => $date));
		$select->order('a.id_admission ASC');
	...
	public function listePatientsConsMedecin($idService){
		...
		$select->join(array('c' => 'consultation'), 'p.ID_PERSONNE = c.ID_PATIENT', array('Id_cons' => 'ID_CONS', 'Dateonly' => 'DATE_CONS' ));
		//$select->join(array('cons_eff' => 'consultation_effective'), 'cons_eff.ID_CONS = c.ID_CONS' , array('*'));
	    $select->join( array('e1' => 'employe'), 'e1.id_personne = c.ID_MEDECIN' , array('*'));
		//$select->join(array('s' => 'service'), 'c.ID_SERVICE = s.ID_SERVICE', array('Nomservice' => 'NOM'));
		$where = new Where();
		//$where->equalTo('s.ID_SERVICE', $idService);
		$where->notEqualTo('DATE_CONS', $date);
		$select->where($where);
		$select->order('c.DATE_CONS DESC');
		$select->group('c.ID_PATIENT');
